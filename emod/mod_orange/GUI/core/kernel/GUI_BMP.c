...
static int BMP_DrawARGB(__u32 *pData, int x0, int y0, int XSize, int YSize,
                        __u32 *pAddr, int dstlayer_w, int dstlayer_h)
{
    int         x, y, PixelPerLine;
    __u32       l_startaddr;                    /*行起始地址*/
    __u32       col_addr;                       /*列写入地址*/
    __u32       dir;
    __u8        *pColor;
    __u32       *addr;
    __u32       line_stride;
    ...
    for (y = 0; y < YSize; y++)
    {
        ...
        if(dir == GUI_SCNDIR_ROTATE90)
        {
            if(y == 0)
            {
                l_startaddr = (__u32)(pAddr + (dstlayer_w - 1 - x0) * dstlayer_h + (y0));
            }
            else
            {
                l_startaddr += 4;
        ...
        else if(dir == GUI_SCNDIR_ROTATE180)
        {
            if(y == 0)
            {
                l_startaddr = (__u32)(pAddr + (dstlayer_h - 1 - y0) * dstlayer_w  + (dstlayer_w - 1 - x0));
            }
            else
            {
        ...
        else if(dir == GUI_SCNDIR_ROTATE270)
        {
            if(y == 0)
            {
                l_startaddr = (__u32)(pAddr + x0 * dstlayer_h  + (dstlayer_h - 1 - y0));
            }
            else
            {
                l_startaddr -= 4;
        ...
        else
        {
            if(y == 0)
            {
                l_startaddr  = (__u32)(pAddr  + y0 * dstlayer_w + x0);
            }
            else
            {
                l_startaddr  += line_stride;
...
int GUI_BMP_DrawEx(const void *pBMP, int x0, int y0)
{
    ...
    case 32:
        if(alphaEn == BMP_ALPHA_ENABLE)
        {
            Ret = BMP_DrawARGB((__u32 *)pSrc, x0, y0, Width, Height,
                               (__u32 *)orange.fbaddr[GUI_Context.SelLayer],
