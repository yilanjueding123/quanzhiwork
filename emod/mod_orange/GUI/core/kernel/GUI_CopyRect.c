                            ...
                            , GUI_RECT *srcrect, GUI_RECT *dstrect)
{
    ...
    src_addr    =  (void *)((__u32)srcbuf + srcfb_width * srcy_off + ((srcx_off * bpp + 7) / 8));   /*get source address */

    /*get framebuffer destination address */
    dst_addr    =  (void *)((__u32)dstbuf + dstfb_width * dsty_off + ((dstx_off * bpp + 7) / 8));

    for(i = 0; i < height; i++)
    {
        orange_memcpy(dst_addr, src_addr, width);

        src_addr = (void *)((__u32)src_addr + srcfb_width);
        dst_addr = (void *)((__u32)dst_addr + dstfb_width);
    }
