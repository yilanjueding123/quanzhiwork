...
__u32 ListLayer_AdjustFb(LIST_LAYER_MAN_t *plist_layer, __u32 item_w, __u32 foucs_w)
{
    ...
    if(plist_layer->fb_depth >= 4)
    {
        ...
        plist_layer->listbuffer->ebuffer         = (void *)((__u32)plist_layer->listbuffer->buffer + plist_layer->listbuffer->vsize);
    }
    else
    {
                ...
                >= plist_layer->scnrect.height)
        {
            ...
            plist_layer->scn_width = eLIBs_fioctrl(disp, DISP_CMD_SCN_GET_WIDTH, SEL_SCREEN, 0); //modified by Derek,2010.12.07.15:05
            plist_layer->scn_height = eLIBs_fioctrl(disp, DISP_CMD_SCN_GET_HEIGHT, SEL_SCREEN, 0);	//modified by Derek,2010.12.07.15:05
            eLIBs_fclose(disp);
            ...
            plist_layer->listbuffer->buffer          = (void *)((__u32)plist_layer->listbuffer->buffer + plist_layer->listbuffer->esize);
