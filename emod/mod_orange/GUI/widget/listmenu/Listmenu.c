...
static int ListMenuCtrlProc (__gui_msg_t  *msg);
...
void GUI_LstmRxThread(void *p_arg)
{
    ...
    while(1)
    {
        ...
        if(err != OS_NO_ERR)
        {
            SYSTEM_PRINTF("received msg err = %x!\n", err);
        }
        else
        {
...
__listmenu_data_t *GUI_LstmPrivCreate(H_WIN hListmenu)
{
    ...
    if(listmenu_priv->nitem != 0)
    {
        ...
        {
            __msg("foucs width too large width = %d,f_w = %d!\n", listmenu_priv->items[0].item_w_unfocs,
...
#endif
    /* 创建rxtx守护线程             */
    plistmenu->listmenu_RxTID           = esKRNL_TCreate(GUI_LstmRxThread, (void *)plistmenu, 0x800, 2);
#ifdef SIM_PC_WIN
    __RESTORE_MOD_EVN__
#endif
    ...
    for(num = 0; num < listmenu_priv->nitem; num++)
    {
        ...
        if(!menuitem)
        {
            ORANGE_WRN("create menuitem %d failed!\n", num);
...
void  GUI_LstmPostRunMsg(__listmenu_data_t *plistmenu, __u32 runid)
{
    ...
    esKRNL_QPost(plistmenu->listmenu_RxQ, (void *)&plistmenu->msgbuff[plistmenu->listlayer->msgcount % LISTMENU_FRM_RXQ_DEPTH]);
}
    ...
    if(plistmenu->listmenu_RxTID)
    {
        while(1)
        {
            ...
            if(qdata.OSNMsgs == 0)
            {
                ...
                esKRNL_QPost(plistmenu->listmenu_RxQ, &default_msg);
            }
        }
    }
    /*listlayer destory*/
...
__u32 GUI_LstmTouchDown(__listmenu_data_t *pData, __gui_msg_t *msg)
{
...

    {
        ...
        notify_msg.dwReserved   = GUI_ItemP2H(pData->listlayer->menulist->CurItem);
