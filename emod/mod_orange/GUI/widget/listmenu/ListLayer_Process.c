...
__u32  ListLayer_MenuItemCopy(LIST_LAYER_MAN_t    *listlayer, __u32 srcfbno, __u32 dstfbno, __u32 src_x, __u32 src_y, __u32 dst_x, __u32 dst_y)
{
    ...
    else if(listlayer->dir == GUI_SCNDIR_ROTATE90)
    {
        ...
        fb_width = (listlayer->listbuffer->rectsz.height * listlayer->menulist->bpp + 7) >> 3;
        for(i = 0; i < height; i++)
        {
            sysmemcpy(dst_addr, src_addr, width);

            src_addr = (void *)((__u32)src_addr + fb_width);
            dst_addr = (void *)((__u32)dst_addr + fb_width);
        }
    }
...
void ListLayer_FoucsItemThread(void *p_arg)
{
    ...
    while(1)
    {
        ...
        esKRNL_FlagPend(listlayer->foucs_flags, LISTLAYER_FOUCS_FLAG + LISTLAYER_DRAW_ITEM_FLAG,
                        ((OS_FLAG_WAIT_SET_ANY) << 16) | 0, &err);
        if(esKRNL_TDelReq(EXEC_prioself) == OS_TASK_DEL_REQ)
        {
            esKRNL_TDel(EXEC_prioself);
            return ;
        ...
        else if(flags & LISTLAYER_DRAW_ITEM_FLAG)
        {
            /*扩散加载处理*/
            if(listlayer->mode != FIXED_MODE)
            {
