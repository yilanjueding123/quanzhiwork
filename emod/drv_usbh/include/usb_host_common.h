...
struct usb_virt_bus
{
    struct list_head  dev_list;		//设备list
    struct list_head  drv_list;		//驱动list
...
{
    int (*probe_ext) (struct usb_host_virt_sub_dev *dev);	//对usb_host_func_drv{}的probe的extend
    //目前为_usb_core_func_drv_probe_interface()
    int (*disconnect_ext) (struct usb_host_virt_sub_dev *dev);	/* 指向真实的drv */
};
...
struct usb_host_func_drv
{
    struct list_head  virt_dev_list;  /* 与本驱动关联的virt_dev */
...
struct urb
{
    ...
    struct list_head *wrapper_urb;				//指向本结构的wrapper，

    u32 urb_sn;									//urb sn ,just for debug

    u32 is_busy;								/* flag. urb是否正在被hcd处理, urb dequeue时有用 */
    u32 need_dispose;							/* flag. urb是否需要被hcd处理, urb dequeue时有用 */
};
