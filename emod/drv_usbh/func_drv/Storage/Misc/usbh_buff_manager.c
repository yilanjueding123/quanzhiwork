...
typedef s32 (* buff_blk_func_t)(void *pBuffer, __u32 blk, __u32 n, USB_OS_HANDLE hDev);


/* usbh_temp_buff的最大长度 */
static u32 usbh_temp_buff_max_len(void)
{
 ...
 * 事情发生
 */
static void set_usbh_temp_buff_busy(void *temp_buff)
{
    ...
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        ...
        {
            USB_OS_ENTER_CRITICAL(cpu_sr);
            buff_mgr->buff_array[i].is_busy = 1;
            USB_OS_EXIT_CRITICAL(cpu_sr);
        }
    }
}
...
static void set_usbh_temp_buff_free(void *temp_buff)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    __cpu_sr cpu_sr = 0;
    u32 i = 0;
    ...
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        ...
        {
            USB_OS_ENTER_CRITICAL(cpu_sr);
            buff_mgr->buff_array[i].is_busy = 0;
            USB_OS_EXIT_CRITICAL(cpu_sr);
        }
    }
}
...
*********************************************************************
*/
static s32 set_usbh_temp_buff_valid(const void *buff, u32 dev_id, u32 start_lba, u32 sector_size)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    u32 i = 0;
    __cpu_sr cpu_sr = 0;

    if(buff == NULL || sector_size == 0)
    {
        DMSG_PANIC("ERR: set_usbh_temp_buff_valid: input error\n");
        DMSG_PANIC("ERR: buff = 0x%x, sector_size = 0x%x\n", buff, sector_size);
        return -1;
    }
    ...
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        ...
        {
            USB_OS_ENTER_CRITICAL(cpu_sr);
            buff_mgr->buff_array[i].dev_id        = dev_id;
            buff_mgr->buff_array[i].sector_size   = sector_size;
            buff_mgr->buff_array[i].start_lba     = start_lba;
            buff_mgr->buff_array[i].end_lba       = start_lba + (buff_mgr->buff_array[i].buff_len / sector_size) - 1;

            buff_mgr->buff_array[i].used_time     = 1;  /* 第一次被fs使用 */

            buff_mgr->buff_array[i].is_valid      = 1;
            USB_OS_EXIT_CRITICAL(cpu_sr);
            /*
...
***************************************************************************
*/
static s32 set_usbh_temp_buff_invalid(u32 dev_id, u32 lba)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    u32 i = 0;
...
*********************************************************************
*/
static void *select_invalid_usbh_temp_buff(void)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    void *temp_buff = NULL;
    u32 min_used = 0xffffffff;       //最小的使用次数
    u32 buff_num = 0;               //被选中的buff的号
    u32 i = 0;
...
static __s32 select_best_buffer(u32 dev_id, u32 lba, u32 size, __u32 *nr)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    __u32 max_len = 0;  /* 命中buffer里存放的最大数据长度 */
    __u32 i = 0;
...
******************************************************************************
*/
static __s32 read_usbh_temp_buff(u32 dev_id, u32 lba, u32 size, void *buff, __u32 is_copy)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    __u32 buff_start   		= 0;  /* 数据在buff的起始地址 		*/
    __u32 valid_sector 		= 0;  /* buffer里的效数据长度 		*/
    __u32 max_valid_sector  = 0;  /* buffer里的最大效数据长度 	*/
    __u32 buff_len     		= 0;  /* 本次读写数据的长度 		*/
    __u32 buff_no     		= 0;
    ...
    if(select_best_buffer(dev_id, lba, size, &buff_no) == 0)
    {
        ...
        if(is_copy)
        {
            ...
            USB_OS_MEMCPY(buff, (void *)((u8 *)(buff_mgr->buff_array[buff_no].buff) + buff_start), buff_len);
...
******************************************************************************
*/
static void *write_usbh_temp_buff(u32 dev_id, u32 lba, u32 size, const void *buff)
{
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    u32 buff_start  = 0;  /* 数据在buff的起始地址 */
    u32 buff_len    = 0;  /* 本次读写数据的长度 */
    u32 i           = 0;
    ...
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        ...
        if((dev_id == buff_mgr->buff_array[i].dev_id) && buff_mgr->buff_array[i].is_valid)
        {
                    ...
                    && (lba < buff_mgr->buff_array[i].end_lba))
            {
                ...
                USB_OS_MEMCPY((void *)((u8 *)(buff_mgr->buff_array[i].buff) + buff_start), buff, buff_len);

                //这里返回blk的偏移地址
                //return buff_mgr->buff_array[i].buff;
                return (void *)((u8 *)(buff_mgr->buff_array[i].buff) + buff_start);
            }
        }
    }
...
s32 usbh_msc_special_read(void *pBuffer,
                          ...
                          void *blk_read_entry)
{
    __u8 *Buffer_temp 	= (__u8 *)pBuffer;
    __u32 temp_sector_1 = 0;   			//本次所要读写的数据长度
    __u32 total_len     = 0;   			//本次所要读写的数据长度
    __u32 left_sector   = 0;			/* 剩余扇区个数	 */
    //	__u32 sector_nr     = 0;  			//本次成功读取的扇区个数
    ...
    if(total_len < usbh_temp_buff_max_len())
    {
        ...
        */
        ...
        //--<3-4>--把从设备读数据, 传递给fs
        USB_OS_MEMCPY(pBuffer, usbh_temp_buff, total_len);

        DMSG_TEMP_TEST("MEMCPY1: pBuffer = %x,pBuffer_s = %x, pBuffer_e = %x\n",
                       (u32)pBuffer, Buffer_temp, Buffer_temp + total_len);
    ...
    else
    {
        ...
        else
        {
            ...
            //--<3-3>--设置这个buff有效
            set_usbh_temp_buff_valid(usbh_temp_buff, dev_id, (blk + temp_sector_1), secter_size);

            //--<3-4>--把从设备读数据, 传递给fs
            USB_OS_MEMCPY(Buffer_temp + (temp_sector_1 * secter_size),
                          usbh_temp_buff,
...
{
    __u8 *Buffer_temp = (__u8 *)pBuffer;
    u32 this_len = 0;   //本次所要读写的数据长度
    u32 sector_nr = 0;  //本次成功读取的扇区个数
    void *usbh_temp_buff = NULL;   //usbh_temp_buff
    usbh_buff_manager_t *buff_mgr = &usbh_buff_manager;
    buff_blk_func_t buff_blk_read;
    ...
    if(this_len <= usbh_temp_buff_max_len())
    {
        ...
        */
        ...
        //--<3-4>--把从设备读数据, 传递给fs
        USB_OS_MEMCPY(pBuffer, usbh_temp_buff, this_len);
                           ...
                           void *blk_write_entry)
{
    ...
    if(this_len < usbh_temp_buff_max_len())
    {
        ...
        USB_OS_MEMCPY(usbh_temp_buff, pBuffer, this_len);
...
s32 init_usbh_buff_manager(void)
{
    ...
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        buff_mgr->buff_array[i].buff = (void *)USB_OS_PAGE_MALLOC(page);
        if(buff_mgr->buff_array[i].buff == NULL)
        {
            //分配失败,就放弃使用缓冲机制
        ...
        USB_OS_MEMSET((void *)buff_mgr->buff_array[i].buff, 0, buff_mgr->temp_buff_len);
...
failed:
    for(i = 0; i < buff_mgr->temp_buff_nr; i++)
    {
        ...
        {
            USB_OS_PAGE_FREE(buff_mgr->buff_array[i].buff, page);
