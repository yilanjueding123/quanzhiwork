...
typedef __packed struct _usbHidClassDescriptor
{
    __u8  bDescriptorType;		/* Report descriptor type 				*/
    __u16 wDescriptorLength;	/* Total length of report descriptor 	*/
} usbHidClassDescriptor_t;

typedef __packed struct _usbHidDescriptor
{
    __u8  bLength;					/* size of this descriptor 							*/
    __u8  bDescriptorType;			/* Hid decriptor type 								*/
    __u16 bcdHID;					/* Hid Class Specification release number in BCD 	*/
    __u8  bCountryCode;				/* Hardware target country 							*/
    __u8  bNumDescriptors;			/* Number of HID class descriptor to follow 		*/
...
{
    __u32  format;			/* Short items or long items 								*/

    __u8      size;			/* Numeric expression specifying size of data 				*/
    __u8      type;			/* Numeric expression identifying type of item 				*/
    __u8      tag;			/* Numeric expression specifying the function of the item	*/
...
typedef __packed struct _usbHidGlobalItems
{
    __u32 UsagePage;			/* 用途类页 					*/

    __s32 LogicalMinimum;		/* 最小逻辑值 					*/
    __s32 LogicalMaximum;		/* 最大逻辑值 					*/
    __s32 PhysicalMinimum;		/* 最小物理值 					*/
    __s32 PhysicalMaximum;		/* 最大物理值 					*/
    __s32 UnitExponent;			/* 单位指数 					*/
    __u32 Unit;					/* 单位 						*/
    __u32 ReportSize;			/* 报告大小 					*/
    __u32 ReportId;				/* 报告ID 						*/
    __u32 ReportCount;			/* 报告的个数 					*/
    __u32 Push;					/* 项目状态表暂存到stack 		*/
    __u32 Pop;					/* 取回stack最顶层的项目状态表 	*/
...
{
    __u32 usage[USB_HID_MAX_USAGES]; 				/* usage array */
    __u32 collection_index[USB_HID_MAX_USAGES]; 	/* collection index array */
    __u32 usage_index;
    __u32 usage_minimum;
    __u32 delimiter_depth;
    __u32 delimiter_branch;
} usbHidLocalItems_t;

/* Collection Items */
typedef __packed struct _usbHidCollectionItems
{
    __u32 Type;		/* type of collection */
    __u32 Usage;	/* 用途类页 */
    __u32 Level;	/* the level of collection items */
} usbHidCollectionItems_t;

typedef struct _usbHidUsage
{
    __u32 hid;				/* hid usage code, 高16为usage page, 低16位usage */
    __u32 collection_index;	/* index into collection array 	*/
