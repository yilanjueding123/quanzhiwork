...
struct usb_host_virt_dev *usb_host_alloc_virt_dev(struct usb_host_virt_dev *parent, struct usb_virt_bus *bus, u32 port1)
{
    ...
    if(bus == NULL)
    {
        DMSG_PANIC("PANIC : usb_host_alloc_virt_dev()  : input == NULL\n");
        return NULL;
    }

    dev = USB_OS_MALLOC(sizeof(struct usb_host_virt_dev), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
    if (!dev)
    {
        return NULL;
    }

    USB_OS_MEMSET(dev, 0, sizeof(struct usb_host_virt_dev));
    ...
    */
    ...
    dev->usb_virt_dev_semi =  USB_OS_SEMI_CREATE(1);
    //=================================
    //初始化virt_sub_dev部分
    for(i = 0; i < USB_MAX_VIRT_SUB_DEV_NR; i ++)
    {
        dev->virt_sub_dev_array[i].usb_virt_sub_dev_semi = USB_OS_SEMI_CREATE(1);
    }
...
void usb_host_free_virt_dev(struct usb_host_virt_dev *virt_dev)
{
    ...
    if(virt_dev == NULL)
    {
        DMSG_PANIC("PANIC : usb_host_free_virt_dev() : input == NULL\n");
        return ;
    }
    ...
    for(i = 0; i < USB_MAX_VIRT_SUB_DEV_NR; i ++)
    {
        ...
        {
            USB_OS_SEMI_DELETE(virt_dev->virt_sub_dev_array[i].usb_virt_sub_dev_semi, &err);
        }
        else
        {
            DMSG_PANIC("PANIC : usb_host_free_virt_dev() : virt_sub_dev`s semi == NULL\n");
            return;
        }
    }
    ...
    {
        USB_OS_SEMI_DELETE(virt_dev->usb_virt_dev_semi, &err);
    }
    else
    {
        DMSG_PANIC("PANIC : usb_host_free_virt_dev() : virt_dev`s semi == NULL\n");
        return;
    }
    ...
    {
        USB_OS_FREE(virt_dev->manufacturer);
        ...
        for(j = 0; j < virt_dev->descriptor.bNumConfigurations ; j++)
        {
            for( k = 0; k < virt_dev->config[j].desc.bNumInterfaces ; k++)
            {
                for(l = 0; l < virt_dev->config[j].intf_cache[k]->num_altsetting; l++)
                {
                    ...
                    {
                        USB_OS_FREE(virt_dev->config[j].intf_cache[k]->altsetting_array[l].endpoint);
                ...
                {
                    USB_OS_FREE(virt_dev->config[j].intf_cache[k]->altsetting_array);
                ...
                {
                    USB_OS_FREE(virt_dev->config[j].intf_cache[k]);
                ...
                DMSG_INFO("new_interfaces[n]--free-- = %x\n",virt_dev->config[j].interfac[k]);
                if(virt_dev->config[j].interfac[k]){
                	USB_OS_FREE(virt_dev->config[j].interfac[k]);
            ...
            //free
            //USB_OS_FREE(virt_dev->config->intf_cache);
    ...
    for( i = 0; i < virt_dev->descriptor.bNumConfigurations; i++)
    {
        ...
        {
            USB_OS_FREE(virt_dev->rawdescriptors[i]);
    ...
    {
        USB_OS_FREE(virt_dev->rawdescriptors);
...
s32 _usb_core_func_drv_probe_interface(struct usb_host_virt_sub_dev *sub_dev)
{
    ...
    {
        DMSG_PANIC("PANIC : _usb_core_func_drv_probe_interface() : input fail,sub_dev = 0x%x ,\n");
        return  -ENODEV;
    }
    ...
    if (!func_drv->probe)
    {
        DMSG_PANIC("PANIC : _usb_core_func_drv_probe_interface() : func_drv->probe == NULL\n");
        return error;
    }
    ...
    if(virt_dev == NULL)
    {
        DMSG_PANIC("PANIC : virt_dev == NULL\n");
        return error;
    }

    if (virt_dev->state == USB_STATE_SUSPENDED)
    {
        DMSG_PANIC("PANIC : virt_dev->state == USB_STATE_SUSPENDED\n");
        return -EHOSTUNREACH;
    }

    DMSG_PANIC("----------------------------------\n");
    DMSG_PANIC("_usb_core_func_drv_probe_interface\n");
    DMSG_PANIC("----------------------------------\n");
