...
s32 hub_port_status(struct usb_hub *hub, s32 port1, u16 *status, u16 *change)
{
    ...
    if (ret < 0)
    {
        DMSG_PANIC ("hub_port_status() : failed (err = %d)\n",  ret);
    }
    else
    {
...
void hub_quiesce(struct usb_hub *hub)
{
    if(hub == NULL)
    {
        DMSG_PANIC("ERR: hub_quiesce: input error\n");
        return;
    }
                   ...
                   u16 *status, u16 *change)
{
    ...
    if (ret < 0)
    {
        DMSG_PANIC("PANIC : hub_hub_status()  failed (err = %d)\n", ret);
    }
    else
    {
...
int hub_thread_sleep(struct hub_thread_context *thread_cont)
{
    ...
    {
        DMSG_PANIC("PANIC : hub_thread_sleep() : ret = %d\n", ret);
        return -1;
    }
