                    ...
                    u16 value, u16 index, void *data, u16 size, s32 timeout)
{
    ...
    dr = USB_OS_MALLOC(sizeof(struct usb_ctrlrequest), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
    if (!dr)
    {
    ...
    USB_OS_MEMSET(dr, 0, sizeof(struct usb_ctrlrequest));
...
s32 usb_get_descriptor(struct usb_host_virt_dev *dev, u8 type, u8 index, void *buf, s32 size)
{
    ...
    USB_OS_MEMSET(buf, 0, size);	// Make sure we parse really received data
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        {
            DMSG_PANIC("get_descriptor faile 1\n");
            continue;
        }

        if (result > 1 && ((u8 *)buf)[1] != type)
        {
            DMSG_PANIC("get_descriptor faile 2\n");
            result = -EPROTO;
            continue;
        }
                               ...
                               void **ptr)
{
    ...
    while (size >= sizeof(struct usb_descriptor_header))
    {
        ...
        if (header->bLength < 2)
        {
            DMSG_PANIC("bogus descriptor, type %d length %d\n",
...
int usb_string(struct usb_host_virt_dev *dev, int index, char *buf, u32 size)
{
    ...
    {
        DMSG_PANIC("ERR: usb_string, status is invalid\n");
        return -EHOSTUNREACH;
    }

    if (((s32)size) <= 0 || !buf || !index)
    {
        DMSG_PANIC("ERR: usb_string, input error\n");
        return -EINVAL;
    }

    buf[0] = 0;
    tbuf = USB_OS_MALLOC(256, USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
    if (!tbuf)
    {
        DMSG_PANIC("ERR: usb_string, malloc failed\n");
        return -ENOMEM;
    }
    USB_OS_MEMSET(tbuf, 0, 256);
    ...
    if (!dev->have_langid)
    {
        ...
        if (err < 0)
        {
            DMSG_PANIC("PANIC : string descriptor 0 read error: %d\n", err);
            goto errout;
        }
        else if (err < 4)
        {
            DMSG_PANIC("PANIC : string descriptor 0 too short\n");
            err = -EINVAL;
            goto errout;
        }
        else
    ...
    {
        DMSG_INFO("wrong descriptor type %02x for string %d (\"%s\")\n", tbuf[1], index, buf);
    }
...
s32 usb_set_configuration(struct usb_host_virt_dev *virt_father_dev, s32 configuration)
{
    ...
    {
        DMSG_PANIC("ERR : set config fail\n");
        return -EINVAL;
    }
    ...
    if (virt_confg && configuration == 0)
    {
        DMSG_PANIC("ERR : config 0 descriptor??\n");
    }

    if (virt_father_dev->state == USB_STATE_SUSPENDED)
    {
        DMSG_PANIC("ERR: usb_set_configuration: device is suspended\n");
        return -EHOSTUNREACH;
    }
    ...
    if (virt_confg)
    {
        ...
        {
            DMSG_PANIC("PANIC : Set_Conifg : too many interface,now we only support %d interfaces\n", USB_MAX_VIRT_SUB_DEV_NR);
            return -EINVAL;
        }

        new_interfaces = USB_OS_MALLOC(nintf * sizeof(sizeof(struct usb_interface *)), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
        if (!new_interfaces)
        {
            DMSG_PANIC("PANIC : Out of memory");
            return -ENOMEM;
        }
        USB_OS_MEMSET(new_interfaces, 0, nintf * sizeof(sizeof(struct usb_interface *)));

        for (; n < nintf; ++n)
        {
            new_interfaces[n] = USB_OS_MALLOC(sizeof(struct usb_interface), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
            if (!new_interfaces[n])
            {
                DMSG_PANIC("PANIC : Out of memory");
                ret = -ENOMEM;
free_interfaces:
            ...
            USB_OS_MEMSET(new_interfaces[n], 0, sizeof(struct usb_interface));
        }
    }
    ...
    DMSG_PANIC("//usb_set_configuration --<5_1>\n");
    ...
    else
    {
        ...
        for (i = 0; i < nintf; ++i)
        {
            ...
            USB_OS_MEMSET(intf, 0, sizeof(struct usb_interface));
        ...
        {
            virt_confg->string = USB_OS_MALLOC(256, USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
            if (virt_confg->string)
            {
                USB_OS_MEMSET(virt_confg->string, 0, 256);
        ...
        DMSG_PANIC("//usb_set_configuration --<5_2>\n");
        ...
        for (i = 0; i < nintf; ++i)
        {
            ...
            DMSG_INFO("[usbh core]: adding sub dev  (config #%d, interface %d)\n", configuration, intf->altsetting [0].desc.bInterfaceNumber);
            ...
            if(!i)
            {
                ...
                {
                    intf->cur_altsetting->string = USB_OS_MALLOC(256, USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
                    if (intf->cur_altsetting->string)
                    {
                        USB_OS_MEMSET(intf->cur_altsetting->string, 0, 256);
            ...
            DMSG_PANIC("//usb_set_configuration --<5_3>\n");
            ...
            if (ret != 0)
            {
                DMSG_PANIC("PANIC : usb_virt_bus_dev_add() fail ret = %d\n", ret);

                if(intf->cur_altsetting->string)
                {
                    USB_OS_FREE(intf->cur_altsetting->string);
...
s32 usb_get_device_descriptor(struct usb_host_virt_dev *dev, u32 size)
{
    ...
    desc = USB_OS_MALLOC(sizeof(struct usb_device_descriptor), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );
    if (!desc)
    {
        DMSG_PANIC("ERR: malloc failed\n");
        return -ENOMEM;
    }
    USB_OS_MEMSET(desc, 0, sizeof(struct usb_device_descriptor));
...
{
    s32 ret = 0;
    u16 *status = USB_OS_MALLOC(sizeof(*status), USB_MEM_FILE_TRIGGER , USB_MEM_LINE_TRIGGER );

    if (!status)
    {
        DMSG_PANIC("ERR:　usb_get_status, malloc failed\n");
        return -ENOMEM;
    }
    USB_OS_MEMSET(status, 0, sizeof(*status));
...
int usb_set_interface(struct usb_host_virt_dev  *dev, int interface, int alternate)
{
    ...
    if (!iface)
    {
        DMSG_PANIC("selecting invalid interface %d\n", interface);
        return -EINVAL;
    }
    ...
    if (!alt)
    {
        DMSG_PANIC("selecting invalid altsetting %d", alternate);
        return -EINVAL;
    }
    ...
    {
        DMSG_PANIC("manual set_interface for iface %d, alt %d\n", interface, alternate);
        manual = 1;
    }
    else if (ret < 0)
    {
