...
void usb_hcd_poll_rh_status(struct hc_gen_dev *hcd)
{
    ...
    {
        DMSG_PANIC("usb_hcd_poll_rh_status() hcd->uses_new_polling = %d ,hcd->status_urb= %的、",
    ...
    if (length > 0)
    {
        ...
        if (urb)
        {
            if (urb->status == -EINPROGRESS)
            {
                ...
                USB_OS_MEMCPY(urb->transfer_buffer, buffer, length);
            }
...
static int rh_queue_status (struct hc_gen_dev *hcd, struct urb *urb)
{
    ...
    {
        DMSG_PANIC( "not queuing rh status urb\n");
        retval = -EINVAL;
    }
    else
    {
...
static int rh_call_control (struct hc_gen_dev *hcd, struct urb *urb)
{
    ...
    case EndpointRequest | USB_REQ_GET_STATUS:
        ...
        DMSG_INFO ("no endpoint features yet\n");
        break;
...
error:
    ...
    if (status)
    {
        ...
        if (status != -EPIPE)
        {
            DMSG_PANIC("CTRL: TypeReq=0x%x val=0x%x "
                       "idx=0x%x len=%d ==> %d\n",
...
int rh_urb_dequeue (struct hc_gen_dev *hcd, struct urb *urb)
{
    ...
    if (usb_pipeendpoint(urb->pipe) == 0)  	/* Control URB */
    {
        ...
        */
        USB_OS_ENTER_CRITICAL(urb->lock_urb);	/* from usb_kill_urb */
        ++urb->reject;
