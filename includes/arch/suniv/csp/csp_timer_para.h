...
* Note       : If the source is CSP_TMRC_LOSC_SRC_EX_32768, the rate of
            ...
            !Important!The LOSC source in 1619 soc can only be CSP_TMRC_LOSC_SRC_HOSC!!!
            So the HOSC can't be close, or system will be asleep forever.
*********************************************************************/
...
{
    CSP_TMRC_TMR_SRC_LOSC,  //32kHz
    CSP_TMRC_TMR_SRC_HOSC   //24MHz
} CSP_TMRC_TmrSrc_t;
...
typedef struct _CSP_WD_PARA
{
    ...
    __bool            irqEnable;
    __bool            resetValid;
    u32               interVal;////(0:0.5s; 1:1s; 2: 2s; 3:3s; 4:4s; 5:5s; 6:6s; 7:8s; 8:10s; 9:12s; 10:14s;11;16s; )
