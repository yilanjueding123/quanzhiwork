...
void CU_dump_memory_usage(const char *szFilename)
{
    ...
    {
        __err( "Failed to open file \"%s\" : %s", szDumpFileName, strerror(errno));
        return;
    }

    //setvbuf(pFile, NULL, _IONBF, 0);  // grace del
    ...
    //time(&tTime);    // grace del
    //fprintf(pFile, "\n  <MD_FOOTER> Memory Trace for CUnit Run at %s </MD_FOOTER>", ctime(&tTime));   //grace del
