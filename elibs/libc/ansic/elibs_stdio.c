...
int eLIBs_ftruncate(ES_FILE *stream, __u32 length)
{
    return esFSYS_ftruncate((__hdle)stream, length);
}


int eLIBs_fseek(ES_FILE *stream, __s32 offset, int whence)
{
...
**********************************************************************************************************************
*/
int eLIBs_closedir(ES_DIR *pDir)
{
...
{
    int         Charset = EPDK_CHARSET_ENM_UNKNOWN;
    __ES_FSSTAT buf;
...
*******************************************************************************************************************
*/
int eLIBs_printf(const char *format, ...)
{
    va_list args;
    int     rc;

    va_start(args, format);
    rc = eLIBs_vprintf(format, args);
    va_end(args);
    return rc;
}
...
{
    return esTIME_GetTime((__time_t *)time);
}

__s32 eLIBs_GetDate(void *date)
{
    return esTIME_GetDate((__date_t *)date);
}
