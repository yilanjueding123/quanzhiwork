...
{
    com->prog_bmp_hdl = dsk_theme_open(ID_INIT_USB_01_BMP); //   		ID_INIT_POINT_BMP
    com->prog_bmp_hd2 = dsk_theme_open(ID_INIT_USB_02_BMP);
    if( !com->prog_bmp_hdl )
    {
...
static __s32 deinit_prog_dialog_res(pdialog_para_t *com)
{
    if(com && com->prog_bmp_hdl)
    {
        ...
        com->prog_bmp_hdl = NULL;
    }
    if(com && com->prog_bmp_hd2)
    {
        dsk_theme_close(com->prog_bmp_hd2);
        com->prog_bmp_hd2 = NULL;
    }
    return EPDK_OK;
}
...
static __s32 cb_progress_dialog(__gui_msg_t *msg)
{
    ...
    case GUI_MSG_PAINT:
    {
        ...
        /* create font  */
        //SWFFont = GUI_SFT_CreateFont(24, BEETLES_APP_ROOT"res\\fonts\\font24.sft" );
    ...
    case GUI_MSG_TIMER:
    {
        ...
        if( GUI_LYRWIN_STA_SUSPEND != GUI_LyrWinGetSta(layer) )
        {
            ...
            if(com->timer_count == 5)
            {
                GUI_BMP_Draw(dsk_theme_hdl2buf(com->prog_bmp_hdl), 100, 0);
                com->timer_count = 0;
            }
            else if(com->timer_count == 1)
                GUI_BMP_Draw(dsk_theme_hdl2buf(com->prog_bmp_hd2), 100, 0);
            com->timer_count++;
...
H_WIN progress_dialog_create(H_WIN owner, H_WIN layer, pdialog_create_t *para)
{
    ...
    dlg_create.dwAddData	= (__u32)com;
