...
G_FILE  *volatile  robin_hdis       = NULL;
...
__s32 cedar_open( robin_open_arg_t *arg_p)
{
    ...
    if(arg_p)
    {
        ...
        cedar_stop_mode = CEDAR_STOP_MODE_UNINSTALL_PLUGINS; // arg_p->cedar_stop_mode;
        switch_vplay_mode = CEDAR_FILE_SWITCH_VPLY_MODE_SEAMLESS;//arg_p->switch_vplay_mode;
        //__log("-------y---------%d:%d:%d\n",reserve_mem_size,cedar_stop_mode,switch_vplay_mode);
    }
    else
    {
        // __log("-------n---------\n");
    ...
    if( robin_hced == NULL )
    {
        ...
        if(ret == EPDK_FAIL)
        {
            __msg("Error in set reserved memory %d Kbyte\n", reserve_mem_size / 1024);
            return -1;
        }
        else
        {
            __msg("set reserved memory %d Kbyte\n", reserve_mem_size / 1024);
        }
...
{
    robin_cedar_msgQ = (g_Queue) esMODS_MIoctrl( robin_hced, CEDAR_CMD_GET_MESSAGE_CHN, 0, NULL );
