...
//#define __here__   eLIBs_printf("@%s(%d)\n", __FILE__, __LINE__)
...
static __s32 __sub_show_update_subtitle(movie_sub_show_scene_t *scene_para)
{
    ...
    if(-1 == ret)//无字幕
    {
        //eLIBs_printf("@%s(%d)__here__", __FILE__, __LINE__);
    ...
    __msg("%s,%d, sub_index:%d\n", __FUNCTION__, __LINE__, sub_index);
    __here__;
    ...
    else
    {
        ...
        //eLIBs_printf("%s(%d) coding = %d.\n", __FILE__, __LINE__, coding);
...
#if 1
        {
            ...
            len = eLIBs_strlen(scene_para->sub_inf.sub_item->pSubtitle);
            for(i = 0; i < len; i++)
            {
                eLIBs_printf("%2x ", scene_para->sub_inf.sub_item->pSubtitle[i]);
            }
