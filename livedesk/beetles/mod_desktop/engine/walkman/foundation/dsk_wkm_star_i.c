...
****************************************************************************************************
*/
__s32  __dsk_wkm_set_star( const char *file, __u32 star )
{
    __u32  UID;
    __s32  ret;

    ret = g_fioctrl( dsk_wkm_horc, ORC_CMD_ADD_FILE_TO_ORCHID, (__s32)file, &UID );
    if( ret != ORC_OK )
    {
    ...
    ret = g_fioctrl( dsk_wkm_horc, ORC_CMD_SET_STAR, (__s32)UID, (void *)star );
    if( ret != ORC_OK )
    {
        __wrn("Fail in setting star %d to file %s.\n", star, file);
        return -1;
