...
typedef	struct ScanStackEntry
{
    ...
    __u8	 Path[1];	 	/* full path name, the length depend on Reclen,
							 * NOTE : Path must been located at the last		*/
} ScanStackEntry_t;
...
typedef	struct ScanStack
{
    ...
    __u8   *Data;			/* data area to store stack entry,
							 * data area size will be gown if necessary */
} ScanStack_t;
